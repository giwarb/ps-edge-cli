...
| dialog | `dialog` / `dialog -Accept [-Text <reply>]` / `dialog -Dismiss` | Two layers prevent hangs: the injected page hook is installed recursively in same-origin iframes as the fast path, while both CDP receive loops auto-handle `Page.javascriptDialogOpening` from persisted policy and report native answers as `# dialog: ...` footer lines. Cross-origin iframes use that CDP safety net. Session connect pre-clears an already-open dialog before `Page.enable`, because enabling the Page domain can time out while a dialog is open; `beforeunload` is always accepted so navigation proceeds. |
